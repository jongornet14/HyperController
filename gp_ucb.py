...
def _select_config(
    ...
    if current is None:
        ...
        # lengthscale=1.)
        kernel = TV_SquaredExp(
            input_dim=X.shape[1], variance=1.0, lengthscale=1.0, epsilon=0.0
        )
...
class GP_UCB():
    ...
    def __init__(self,bounds):
        ...
        for key in bounds.keys():
            type_ = type(bounds[key][0])
            self.config[key] = type_((bounds[key][1]-bounds[key][0])*np.random.rand()+bounds[key][0])
    ...
    def reset_values(self):
        ...
        lst       = [['GP-UCB'] + [0] + list(self.config.values()) + [np.random.randn()]]
        cols      = ["Trial", "Time"] + list(self.bounds) + ["Reward"]
        new_entry = pd.DataFrame(lst, columns=cols)
        self.data = pd.concat([self.data, new_entry]).reset_index(drop=True)

    def update_data(self,X):
        
        lst       = [['GP-UCB'] + [time.time()] + list(self.config.values()) + [X]]
        cols      = ["Trial", "Time"] + list(self.bounds) + ["Reward"]
        new_entry = pd.DataFrame(lst, columns=cols)
        self.data = pd.concat([self.data, new_entry]).reset_index(drop=True)
    ...
    def make_choice(self):
        ...
        self.config,self.data = _explore(self.data,self.bounds,current,base,old,self.config)
